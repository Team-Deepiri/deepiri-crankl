...
INPUT="${1:?usage: post_train_crankle.sh weights.f32 output.cran [manifest.json]}"
...
if [[ -n "$MANIFEST" ]]; then
    "$CLI" finetune --input "$TMP" --target "$INPUT" --steps 100 --lr 0.03 -o "$OUTPUT" \
        | tee /tmp/crankle_post_train.log
    python3 - "$MANIFEST" /tmp/crankle_post_train.log <<'PY'
import json, re, sys
...
with open(manifest, "w") as f:
    json.dump({"tool": "crankle", "stage": "post_train", "metrics": metrics}, f, indent=2)
PY
else
    "$CLI" finetune --input "$TMP" --target "$INPUT" --steps 100 --lr 0.03 -o "$OUTPUT"
fi
