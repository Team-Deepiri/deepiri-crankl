...
MetricsPanel::MetricsPanel(QWidget *parent) : QFrame(parent) {
    ...
    m_unavailableNote =
        new QLabel(tr("Metrics could not be computed for this archive. The fields above are "
                      "not measurements — treat them as missing, not as zero."),
                   this);
    m_unavailableNote->setObjectName(QStringLiteral("MetricsEmptyNote"));
    m_unavailableNote->setWordWrap(true);
    layout->addWidget(m_unavailableNote);
...
void MetricsPanel::setUnavailable() {
    for (const Row &row : m_rows)
        row.value->setText(tr("unavailable"));
    applyState(State::Unavailable);
}
...
void MetricsPanel::applyState(State state) {
    ...
    m_unavailableNote->setVisible(state == State::Unavailable);
}
