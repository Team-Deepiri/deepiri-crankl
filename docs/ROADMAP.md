...
| v0.4 | Crankle Flow + decrank-unified finetune: inspect, compare, pipeline manifests, Maurer-Cartan turn | **in progress** |
...
3. **`crankle finetune`** — reconstruction + holonomy MSE, layer stack persistence.
4. **Peel stacks** — cran v2 layer history rollback via `peel --layers N`.
5. **Post-train hooks** — `bench_finetune.sh`, `post_train_crankle.sh`.
