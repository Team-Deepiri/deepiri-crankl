...
discrete Lie-group optimization loop — no float backprop through Crankle.
...
crankle finetune --input adapter.cran --target weights.f32 \
...
crankle peel --input tuned.cran --layers 1 -o rolled_back.cran
```
...
| Mode | Command | Output |
|------|---------|--------|
| Decrank (default) | `crankle unpack --input a.cran -o out.f32` | 64 floats per slot |
| Legacy coeffs | `crankle unpack --unpack-mode coeffs ...` | 8 multivector coeffs per slot |
...
int crankle_finetune(uint64_t *slots, size_t n_slots, crankle_cran_t *cran_view,
                     const float *target_blocks, crankle_loss_fn task_loss, void *ctx,
...
./scripts/post_train_crankle.sh weights.f32 tuned.cran manifest.json
```
