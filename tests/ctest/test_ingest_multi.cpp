...
std::string make_temp_path(const char *suffix) {
    ...
    return std::filesystem::temp_directory_path().string() + "/crankl_test_ingest_" + std::to_string(std::hash<std::thread::id>{}(std::this_thread::get_id())) + "_" + std::to_string(counter++) +
           suffix;
}
