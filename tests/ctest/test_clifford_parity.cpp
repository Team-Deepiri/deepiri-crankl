...
static uint64_t from_mv(const double m[8], uint8_t depth = 1) {
    ...
    mv.p = m[7];
    return crankle_crank_from_multivector(&mv, depth);
}
...
int main() {
    ...
    for (int i = 0; i < CLIFFORD_CASE_COUNT; ++i) {
        ...
        double r = crankle_clifford_resonance(wa, wb);
    ...
    crankle_multivector_t e1{0, {1, 0, 0}, {0, 0, 0}, 0};
    crankle_multivector_t e2{0, {0, 1, 0}, {0, 0, 0}, 0};
    crankle_multivector_t e12{0, {0, 0, 0}, {1, 0, 0}, 0};
    crankle_multivector_t prod{};

    crankle_clifford_product(&e1, &e1, &prod);
    if (std::fabs(prod.s - 1.0) > 1e-6) {
        std::fprintf(stderr, "FAIL e1*e1 scalar=%g\n", prod.s);
        ++fails;
    }
    crankle_clifford_product(&e1, &e2, &prod);
    if (std::fabs(prod.b[0] - 1.0) > 1e-6) {
        std::fprintf(stderr, "FAIL e1*e2 e12=%g\n", prod.b[0]);
        ++fails;
    }
    crankle_clifford_product(&e12, &e12, &prod);
    if (std::fabs(prod.s + 1.0) > 1e-6) {
